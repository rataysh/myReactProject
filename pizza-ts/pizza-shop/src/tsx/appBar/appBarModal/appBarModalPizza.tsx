...
export const ModalPizza: React.FC<IModalPizza> = (props) => {
    ...
    let descriptionSize: string =
        props.templateModalPizza.size === "sm"
            ? "Small"
            : props.templateModalPizza.size === "lg"
            ? "Large"
            : "Standart";

    return (
        <>
            <div className='relative my-1 pb-2 pt-1 px-2 lg:px-4 w-full flex-col items-center border rounded-xl bg-slate-50 lg:text-lg md:text-lg lg:font-medium md:font-medium font-sans'>
                <div className='flex justify-end relative'>
                    <button
                        className='flex absolute z-10 top-0.5'
                        onClick={() =>
                            dispatch(removeItem(props.templateModalPizza))
                        }>
                        <svg
                ...
                <div className='flex justify-between items-center lg:px-5 md:px-3 sm:px-2'>
                    <div className='mt-2'>
                        ...
                        {(
                            (props.templateModalPizza.price +
                                calcSize(props.templateModalPizza)) *
                            (props.templateModalPizza.count ?? 1)
                        ).toFixed(2)}
                    </div>
                    <div className='bg-slate-200 border border-slate-300 rounded-xl mt-2 flex items-center justify-center lg:space-x-2 md:space-x-1'>
                        <button
                            onClick={() =>
                                dispatch(minusItem(props.templateModalPizza))
                            }
                            className={
                                (props.templateModalPizza.count === 1
                                    ? "disabled"
                        ...
                        <div>{props.templateModalPizza.count}</div>
                        <button
                            onClick={() =>
                                dispatch(addItem(props.templateModalPizza))
                            }
